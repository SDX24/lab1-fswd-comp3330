...
const required = ['KINDE_ISSUER_URL','KINDE_CLIENT_ID','KINDE_CLIENT_SECRET','KINDE_REDIRECT_URI'] as const
for (const k of required) {
  if (!process.env[k]) {
    throw new Error(`Missing env ${k}. Set it in .env`)
  }
}
...
export function sessionFromHono(c: any): SessionManager {
  return {
    ...
    async destroySession() {
      for (const k of ['access_token', 'id_token', 'refresh_token', 'session', 'state']) {
