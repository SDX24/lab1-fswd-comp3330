...
export const expensesRoute = new Hono()
  ...
  .post('/', zValidator('json', createExpenseSchema), async (c) => {
    ...
    if (!created) return c.json({ error: 'Failed to create expense' }, 500)
  ...
  .delete('/:id{\\d+}', async (c) => {
    ...
    const expenseWithUrl = await withSignedDownloadUrl(deletedRow)
    return c.json({ deleted: expenseWithUrl })
  })
